...
public class TarjetaCredito implements Serializable {
    ...
    /**
     * Contructor pensado para crear Tarjetas de Credito a partir de datos
     * obtenidos de una base de datos.
     *
     * @param nombre el nombre del titular
     * @param nif el documentod de identificacion nif/cif/nie del titular
     * @param pin el pin de la tarjeta de credito
     * @param limite el limite de gasto de la tarjeta
     * @param numeroTarjeta el numero de la tarjeta de credito
     * @param mes el mes de caducidad
     * @param año el año de caducidad
     * @param cvv el cvv de la tarjeta de credito
     * @throws IllegalArgumentException si El nombre del titular es menor de 5 o
     * mayor de 80 caracteres o no esta compuesto de caracteres alfanumericos
     * @throws IllegalArgumentException si el NIF/CIF/NIE no es un documento que
     * sigue un algoritmo que lo valida
     ...
     * introducido no sigue el algoritmo de Luhn.
     */
    public TarjetaCredito(String nombre, String nif, String pin, int limite, String numeroTarjeta, String mes, String año, String cvv) {
    ...
    public void setLimite(int limite) {
        if (limite >= 500 && limite <= 5000) {
            this.saldo += (this.limite - limite < 0) ? (limite - this.limite) : (this.limite - limite);
            this.limite = limite;
        }
    }
    ...
    public boolean pagar(double cantidad, String concepto) {
        boolean pagoValido = (cantidad > 0 && Pattern.matches(".{0,50}", concepto) && cantidad <= this.saldo);

        if (pagoValido) {
            introducirMovimiento(cantidad, concepto);
            this.saldo = (this.saldo - cantidad <= 0) ? 0 : (int) (this.saldo - cantidad);
        }
    ...
    public void introducirMovimientoBD(double cantidad, String concepto, Date fecha) {
        //creo el nuevo movimiento a introducir
//        YYYY-MM-DD HH:MM:SS.NNNNNNNNN
        this.movimientos.add(new Movimiento(cantidad, concepto, fecha));
